...
/**
 * Legacy REST auth client for the OTP password-reset flow ONLY.
 *
 * The main authentication flow (login / signup / session persistence) now uses
 * the canonical @supabase/ssr browser + server clients. The OTP recovery flow
 * (forgot-password / reset-password) is intentionally left on these dedicated
 * REST wrappers so its behaviour is unchanged.
 */
