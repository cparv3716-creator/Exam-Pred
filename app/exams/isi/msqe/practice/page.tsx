...
const modes = [
  { icon: BookOpenCheck, title: "PEA Objective Practice", description: "Source-preserved PEA questions with clickable option checking.", href: "/exams/isi/msqe/pea" },
  ...
  { icon: Layers3, title: "PYQ Resources", description: "Original source PDFs plus regenerated branded PEA question PDFs.", href: "/exams/isi/msqe/pyqs" },
...
export default function MsqePracticePage() {
  ...
  return <IsiPageShell><section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20"><IsiPageHeader eyebrow="MSQE / Practice" title="Objective speed. Descriptive depth." description="The active PEA practice bank is regenerated from local Statstrive solution TeX sources. Full available question context is retained, and incomplete source items stay visible with a needsReview flag." chips={sets.map((set) => `${set.title}: ${set.questionIds.length}`)} /><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{modes.map(({ icon: Icon, title, description, href }) => <Link key={title} href={href} className="aurora-glass aurora-card-hover aurora-focus-ring flex flex-col p-5"><span className="grid h-10 w-10 place-items-center rounded-xl text-white" style={{ background: "linear-gradient(135deg, var(--aurora-primary), var(--aurora-violet))" }}><Icon size={17} /></span><h2 className="mt-4 text-base font-bold">{title}</h2><p className="mt-2 flex-1 text-xs leading-6" style={{ color: "var(--aurora-text-secondary)" }}>{description}</p><span className="mt-4 inline-flex items-center gap-1 text-xs font-bold" style={{ color: "var(--aurora-primary)" }}>Open <ArrowRight size={13} /></span></Link>)}</div><div id="sample-questions" className="mt-12 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.17em]" style={{ color: "var(--aurora-primary)" }}>Source-preserved practice</p><h2 className="mt-2 text-2xl font-extrabold">PEA 2022-2026 question bank</h2></div><span className="aurora-badge">{questions.length} items</span></div><div className="mt-6 grid gap-5 md:grid-cols-2">{questions.map((question) => <MsqeQuestionCard key={question.id} question={question} />)}</div></section></IsiPageShell>;
}
